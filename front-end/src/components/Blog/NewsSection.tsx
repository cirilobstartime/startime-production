          ...
          {newsItems.map((item) => {
            ...
            return (
              <motion.div
                key={item.id}
                whileHover={{ y: -5 }}
                className="bg-gray-50 overflow-hidden transition-all rounded-[10px] border border-gray-100 shadow-sm flex flex-col h-full"
              >
                <Link
                    key={item.documentId}
                ...
                <div className="p-6 flex flex-col flex-grow">
                  ...
                  <div className="mt-auto">
                    ...
                    <Link
                        className="mt-4 text-[#5A417F] font-semibold group-hover:translate-x-1 transition-transform"
                        href={`/${locale}/blog/${item.documentId}`}
                    >
                        {isAr ? "اقرأ المزيد" : "Read More →"}
                    </Link>
